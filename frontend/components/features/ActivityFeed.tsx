    ...
    <div className="border-b border-gray-100 pb-4">
      ...
      <div className="flex gap-3 mb-3">
        <div className="flex-shrink-0 cursor-pointer" 
          ...
          <BookCover
            key={`main-cover-${showFirstBook.activity_data?.book_id || 'main'}`}
      ...
      {isExpanded && remainingCount > 0 && (
        <div className="space-y-3 pl-4 border-l-2 border-gray-100">
          {activities.slice(1).map((activity, index) => (
            <div key={`activity-${index}-${activity.activity_data?.book_id || index}`} className="flex gap-3">
              <div className="flex-shrink-0 cursor-pointer" 
                   onClick={() => onBookClick(null, activity.activity_data.book_id)}>
                <BookCover
                  key={`cover-${index}-${activity.activity_data?.book_id || index}`}
    ...
    <div className="border-b border-gray-100 pb-4">
      ...
      <div className="flex gap-3">
        <div className="flex-shrink-0 cursor-pointer" onClick={() => onBookClick(reading.book, reading.book.id)}>
          <BookCover
            key={`review-cover-${reading.book?.id || reading.id || 'review'}`}
